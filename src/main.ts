...
const detail = document.getElementById("detail") as HTMLElement | null;

function clampToValidDate(y: number, m: number, d: number) {
  // Menghindari problem 29 Feb di tahun non-kabisat: fallback ke 28 Feb
  const dt = new Date(y, m, d);
  if (dt.getMonth() !== m) return new Date(y, m, 0); // hari 0 = akhir bulan sebelumnya
  return dt;
}
...
function getZodiacWestern(birth: Date) {
...
function getShioChinese(year: number) {
...
function handleChange() {
  ...
  const isToday =
    nextBD.getFullYear() === today.getFullYear() &&
